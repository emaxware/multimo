...
// Keep looping until the process terminates
 ...
 // Wait for specified amount of time
    case WaitForSingleObject(FProcess, DEF_SLEEP) of // Object is signaled (process is finished)
      WAIT_OBJECT_0:
        begin
...
// Resource protection
 ...
 // Duplicate the write handles so they can be inherited
          ...
          else // Failed to duplicate
            result := False;
        end
        else // Failed to create pipes
          result := False;
 // Should we continue?
        if not (result) then
          break;
...
const
  Priorities: array[TThreadPriority] of Integer = (THREAD_PRIORITY_IDLE, THREAD_PRIORITY_LOWEST, THREAD_PRIORITY_BELOW_NORMAL, THREAD_PRIORITY_NORMAL, THREAD_PRIORITY_ABOVE_NORMAL, THREAD_PRIORITY_HIGHEST, THREAD_PRIORITY_TIME_CRITICAL);
begin
...
// Handle the pipe messages
  case Message.Msg of // Pipe output from console
    WM_PIPE_CON_OUT:
 ...
 // Shutdown
    WM_DOSHUTDOWN:
      Stop(Message.WParam);
  else // Call default window procedure
...
// Resource protection
 ...
 // Keep looping until the process terminates
    ...
    begin
 // Wait for specified amount of time
      case WaitForSingleObject(FPI.hProcess, DEF_SLEEP) of // Object is signaled (process is finished)
        WAIT_OBJECT_0:
          begin
 // Process the output pipes one last time
...
// Both params cannot be null
 ...
 // Synchronous loop with no timeout
              ...
              else // Synchronous loop with timeout
...
// Resource protection
 ...
 // Set base local pipe name
        ...
        else // Set base pipe name using specified server
...
// Make sure we are not being called from one of the events
 ...
 // Wait until the empty flag is set or we get a message
      case MsgWaitForMultipleObjects(1, hEvent, False, INFINITE, QS_SENDMESSAGE) of // Empty event is signalled
        WAIT_OBJECT_0:
          break;
...
// Handle the pipe messages
  case Message.Msg of // Pipe worker error
    WM_PIPEERROR_W:
...
// Handle the pipe messages
  case Message.Msg of // Listener thread error
    WM_PIPEERROR_L:
      if Assigned(FOPE) then
...
// Check against current state
 ...
 // Set deferred state
      FDeferActive := Value // Check designing state. The problem is that in the IDE, a count on the
...
// Check pending state
 ...
 // Reset the write event
        ...
        end // Only acceptable error is pending
        else if (FErrorCode = ERROR_IO_PENDING) then
 // Set pending flag
          FPendingWrite := True
        else // Failure
...
// Resource protection
 ...
 // Create new multi part message
        ...
        end // Check to see if this is an end
 ...
 // Add data to existing stream
        ...
        else // Send the message to the notification window
          SafeSendMessage(WM_PIPEMESSAGE, FPipe, Integer(FRcvStream));
      end;
    end // Check to see if we are in a multi part message
...
// Resource protection
 ...
 // Handle the event that was signalled (or failure)
          case WaitForMultipleObjects(dwEvents, @FEvents, False, INFINITE) of // Killed by pipe server
            WAIT_OBJECT_0:
              begin
 // Resource protection
                try
 // Finish any final read / write (allow them a small delay to finish up)
                  if FPendingWrite and (WaitForSingleObject(FEvents[2], DEF_SLEEP) = WAIT_OBJECT_0) then
                    CompleteWrite;
                  if FPendingRead and (WaitForSingleObject(FEvents[1], DEF_SLEEP) = WAIT_OBJECT_0) then
                    CompleteRead;
                finally
...
// Resource protection
 ...
 // Open the temp file
    ...
    else // Failed to get temp filename
 ...
 // Create handle stream
    ...
    else // Create fast memory stream
      FStream := TFastMemStream.Create;
  end;
...
// Resource protection
 ...
 // Delta is 1/4 of desired capacity
      ...
      else // Minimum allocation of 64 KB
        dwDelta := MaxWord;
 // Update by delta
...
// Handle the window message
  case Message of // Exceute the method in thread
    CM_EXECPROC:
      begin
...
{$ENDIF}
 ...
 // Set message result
  ...
  else // Call the default window procedure
...
// Check current thread against the sync thread id
 ...
 // WaitForSingleObject(Handle, 0) even returns WAIT_TIMEOUT for the invalid handle)
  ...
  else // Wait is not being called from base thread id, so use WaitForSingleObject
...
// Check memory, write code from this process
 ...
 // Free allocated memory
  ...
  else // Failed to create remote injected function
    result := 0;
...
// Check handle
 ...
 // Close the handle
  ...
  else // Attempt to use the process handle from the create process call
    TerminateProcess(ProcessHandle, ExitCode);
...
// Initialize the new security descriptor
 ...
 // Set up the security attributes structure
    ...
    else // Failed to init the sec descriptor
      RaiseWindowsError;
  end
  else // Failed to init the sec descriptor
    RaiseWindowsError;
