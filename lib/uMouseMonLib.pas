...
procedure StartMouseMonitor(ACancel:TEvent; AddMessagePump:boolean);
    ...
    begin
      try
                ...
                begin
                  ...
                  begin
                    case Ri.header.dwType of
                      RIM_TYPEMOUSE:
                      begin
                        log('MON: WM_INPUT %d,%d',[ri.mouse.lLastX, ri.mouse.lLastY]);
                      end;
                    end;
                  end;
                end;
        ...
        begin
          ...
          log(lpError, 'Error GetRawInputAPIS: %d %s',[error, SysErrorMessage(error)]);
{$IFDEF ISCONSOLE}
          readln
{$ENDIF}
        end;
        ...
        begin
          result := false;
          var error := GetLastError;
          log(lpError, 'Error registering rawinputdevice: %d %s',[error, SysErrorMessage(error)]);
{$IFDEF ISCONSOLE}
          readln
{$ENDIF}
        end;
        ...
        Log('Starting hook..');
        if not StartCBTHook
//        if not StartMouseHook
(*
        if not StartLLMouseHook(
          Function(
  ...
  *)
        ...
        begin
          result := false;
          var error := GetLastError;
          log('Error setting hook: %d %s',[error, SysErrorMessage(error)]);
{$IFDEF ISCONSOLE}
          readln
{$ENDIF}
        end
